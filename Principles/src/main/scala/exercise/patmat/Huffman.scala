...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    (chars.toSet map ((char: Char) => (char, chars.count(x => x == char)))).toList.sorted
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs sortWith ((it1, it2) => it1._2 < it2._2) map (freq => Leaf(freq._1, freq._2))
  ...
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = trees match {
    case Nil => Nil
    case xs => if (!done(xs)) until(done, merge)(merge(xs)) else xs
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree =
    until(x => singleton(x), x => combine(x))(makeOrderedLeafList(times(chars))).head
